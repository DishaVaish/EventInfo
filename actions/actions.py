...
class ActionFallback(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        # Call GPT-3.5 API for fallback response
        ...
        # Send fallback response to the user
...
BBSEColl = DataBase['bbselinefollowers']
class giveUserEventLink(Action):
    def name(self) -> Text:
...
DOHHColl = DataBase['decept_of_home']
class giveUsereventLink(Action):
    def name(self) -> Text:
        return "say_dohh_details"
    def run(self, dispatcher: CollectingDispatcher,
        tracker: Tracker,
        domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        query={"_id":"eventRules"}
        projection = {"0":1,"1":1,"2":1,"3":1,"4":1,"5":1,"6":1}
        storage = DOHHColl.find(query,projection)
        des
        for i in storage:
             eventrules = i.get("eventRules", {})
             des=eventrules.get("description")
        dispatcher.utter_message(des)